...
export default function AdminOverview() {
    const stats = [
        { name: "Total Revenue", value: "RFW 42.5M", icon: DollarSign, color: "text-green-400", change: "+12%" },
        ...
        { name: "Profile Views", value: "1,240", icon: Activity, color: "text-accent-red", change: "+24%" },
    ];

    return (
        <div className="max-w-6xl mx-auto space-y-8">
            <div>
                <h1 className="text-3xl font-heading font-bold text-foreground mb-2">System Overview</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <motion.div
                        ...
                        className="bg-secondary border border-border rounded-3xl p-6 relative group"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                {/* Recent Orders Table */}
                <div className="lg:col-span-2 bg-secondary border border-border rounded-3xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold text-foreground">Recent Requests</h2>
                        <button className="text-sm text-accent hover:text-foreground transition-colors">View All</button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="text-sm divide-y divide-white/5">
                                {[
                                    { name: "Sarah Connor", type: "Luxury Web Design", budget: "RFW 5M-10M", status: "New" },
                                    { name: "TechCorp Inc.", type: "Full Stack Platform", budget: "RFW 25M+", status: "In Discussion" },
                                    { name: "Michael Smith", type: "Mobile App", budget: "RFW 10M-25M", status: "Proposal Sent" },
                                    { name: "Elena Gray", type: "Web App Maintenance", budget: "<RFW 5M", status: "Rejected" },
                                ].map((req, i) => (
                                    <tr key={i} className="group hover:bg-secondary transition-colors">
                                        <td className="py-4 text-foreground font-medium">{req.name}</td>
                ...
                <div className="bg-secondary border border-border rounded-3xl p-6">
                    <h2 className="text-xl font-bold text-foreground mb-6">Unread Messages</h2>
                    <div className="space-y-4">
                        {[
                            ...
                            <div key={i} className="p-4 rounded-2xl bg-secondary/50 border border-border hover:border-border transition-colors cursor-pointer group">
                                <div className="flex justify-between items-center mb-2">
                                    <h4 className="text-foreground font-medium text-sm group-hover:text-accent transition-colors">{msg.name}</h4>
                    ...
                    <button className="w-full mt-6 py-3 border border-border rounded-xl text-sm font-medium text-foreground hover:bg-secondary transition-colors">
                        Go to Inbox
                    </button>
                </div>
            </div>
